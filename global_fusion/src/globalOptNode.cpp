...
void publish_car_model(double t, Eigen::Vector3d t_w_car, Eigen::Quaterniond q_w_car)
{
    ...
    t_nsec *= 1e9;
    car_mesh.header.stamp = rclcpp::Time(t_sec,t_nsec);
...
{
    //printf("vio_callback! \n");
    double t = pose_msg->header.stamp.sec + pose_msg->header.stamp.nanosec * (1e-9);
    last_vio_t = t;
    ...
    while(!gpsQueue.empty())
    {
        ...
        double gps_t = GPS_msg->header.stamp.sec + GPS_msg->header.stamp.nanosec * (1e-9);
    ...
    foutC.precision(0);
    foutC << pose_msg->header.stamp.sec * 1e9 + pose_msg->header.stamp.nanosec << ",";
    foutC.precision(5);
    foutC << global_t.x() << ","
            << global_t.y() << ","
            << global_t.z() << ","
...
int main(int argc, char **argv)
{
    ...
    string nh = "globalEstimator/";
    pub_global_path = n->create_publisher<nav_msgs::msg::Path>(nh + "global_path", 100);
    pub_global_odometry = n->create_publisher<nav_msgs::msg::Odometry>(nh + "global_odometry", 100);
    pub_car = n->create_publisher<visualization_msgs::msg::MarkerArray>(nh + "car_model", 1000);
    rclcpp::spin(n);
    return 0;
}
